...
Maybe I could remove the uncertainty bounds ?
or remove mix either ?
...
Ian is concerned that we don't cover the full range of potential selection coefficients.
We do 0.1-0.45 whereas it could go up to 1.
He will work on describing it in the paper and then we can decide whether we need to rerun the sensitivity analysis.

~ perhaps put Single locus fitness code in runModel2 into its own function - would make testing easier.
~~ but not starightforward because it needs arrays of h, phi, s & z
~~ I could allow it to work without these arrays if they are not passed
~~ thus allowing it to be called both from within the prog and outside



~ add in pre-run warning if selection coeff > effectiveness


~ write up why having correctDeploy of 0 equates to mosaics ? Effectively it means that hlf of expsoure is set to one insecticide and half to the other.
