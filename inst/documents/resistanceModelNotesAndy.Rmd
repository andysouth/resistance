...
~ perhaps add dominance into the simpfitvis calc ?
...
~ When my head is clear, think through the ppt & the code about the best terminology for the fitness calcs.
...
~ check on my fitness* functions
~ can I also get them to plot ?

~ look at & incorporate Ians comments on paper2
...
To check on :
is dominance dealt with correctly in model i.e. for both insecticides.
...
maybe fitnessGenotype() should be called fitnessNiche() ?

set up some unit tests for the fitness functions

To say to Ian.
~ if there is no more money, all I can do is get paper2 out & tidy & document the code.


Produce some ideas for Ian about how best to spend my time.
...
write protocol for Matt, include that pull request from his fork.

~ look at visualising exposure, effectiveness and dominance fitness patterns as in Ians ppt to IVCC (slide 14).
~ I could start with 1 insecticide ...
~ base it on my killeen coverage app ...
~ from coverage/plot_feeding


try creating a function to plot mean fitness over time
