...
# function to extract just the R allele frequencies at a specified locus, for all scenarios run
# returns a matrix with rows for each generation, and columns for each scenario
# locus is the locus of interest (i.e. 1 or 2)
# max_gen the number of generations (should be the maximum number used across all runs, i.e. the longest any run is set for
# and results.list the list containing the results matrices generated for each scenario run
...
# Produces a matrix with a column for each scenario, giving the generation number at which frequency of R is first >= 0.5
...
for (i in 1:ncol( input ) ){
  ...
  # Prints male frequency of R allele at locus 1 (blue) and locus 2 (green)
  # and same in female at locus 1 (red) and locus 2 (orange)
  genplot <- allele.freq( results.list[[i]] )
  ...
  # locus 1: SS in pink, RS in orange, RR in red
  # locus 2: SS in cyan, RS in dark blue, RR in green
  genplot <- haplotype( genotype.list[[i]] )
  # Saves plot into same directory as code documents
...
if( calibration == 1012 ){
	curtis <- curtis_f2( results.list[[3]], results.list[[1]], results.list[[2]], 1, 2, 3 )
