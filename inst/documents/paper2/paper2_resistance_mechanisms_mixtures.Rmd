...
A major aim of this paper is to us modern techniques to make the modelling accessible to a wider community. The model is implemented in R [@RCoreTeam2016], the code is open-source and hosted on Github [@South2017] including the code to generate both the figures and text of this paper. We provide an on-line user interface [@South2017a] (Fig 2) accessing the same code and enabling readers with no coding experience to change inputs and run the model themselves. 
...
#### Caveats
There is evidence of the involvement of multiple genes in resistance mechanisms[@Ffrench-Constant2004][@Ffrench-Constant2013]. Our model, in common with almost all previous ones [@Bourguet2010], assumes a single ‘resistance’ gene per insecticide. In most cases we believe this is reasonable. Target-site resistance has been shown to be mostly determined by single genes, and control failures in general have mostly been attributed to the effect of single major genes [@Roush1989][@Ffrench-Constant2004][@Ffrench-Constant2013]. Control doses of insecticides in the field are likely to lead to the selection of single major genes whereas the lower doses used to select laboratory strains are more likely to lead to polygenic resistance [@Ffrench-Constant2013]. To represent polygenic resistance a different 'quantitative genetic' approach would be required [@Via1986][@Firko1990]. A recent review of 187 models of the evolution of resistance [@Bourguet2010] did not find any that represented quantitative multiple gene resistance. We are only aware of preliminary published work applying a quantitative genetic approach to assessing insecticide use strategies in the context of polygenic resistance [@Via1986][@Gardner1998]. If it becomes clearer that in some cases polygenic resistance is more of an issue than previously thought then there will be a need for appropriate quantitative genetic models. We would be keen to contribute to that development.
