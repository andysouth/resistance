---
title: "paper1_results_figs_slimmed_50_rr"
output: pdf_document
...
#Fig1 
```{r Fig1, eval=TRUE, echo=FALSE, fig.height=7.5, fig.width=6, fig.show='hold'}
#, fig.align='left'}
...
plotlist[[2]] <- ggplot(data.frame(x=c(0,100),y=c(80,0)), aes(x=x, y=y)) + 
       theme_bw() + 
       ylim(0,100) +
       ...
       geom_label(data=df_boxes,aes(x=x, y=y, label=lab)) +
...
![curtis fig1](pap1_curtis1.png)

# Fig2 replicating Curtis, and now adding in copies of Curtis plots
```{r Fig2, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=8.5, fig.width=6, fig.keep='last'} 
...
```{r Fig2_justNew, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3.5, fig.width=6, fig.keep='last'} 
...
```{r Fig3, eval=TRUE, echo=FALSE, warning=FALSE, fig.height=4.5}
...
```{r Fig4, eval=TRUE, echo=FALSE, warning=FALSE, fig.height=4.5} 
...
```{r Fig5, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=8} 
...
```{r Fig6, eval=TRUE, echo=FALSE, fig.height=7.5, fig.keep='last'} 
...
```{r Fig7, eval=TRUE, echo=FALSE, warning=FALSE, fig.height=5.5} 
...
```{r Fig9, eval=TRUE, echo=FALSE, fig.height=6} 
...
```
Red triangles are those runs in which resistance reached the threshold at the same time in the two strategies (and are repeated in both panels). The red line is a linear model through these same points. 
\pagebreak
...
```{r Fig10, eval=TRUE, echo=FALSE, fig.height=6} 
...
```{r Fig8, eval=TRUE, echo=FALSE, warning=TRUE, message=FALSE, fig.height=7, fig.keep='last'} 
  ...
  for( treeResponse in treeResponses )
  {
    ...
    #adds in no. scenarios correctly and incorrectly classified
    #cex to set textsize (0.7 all text doesn't quite fit in), good that A&B are now same size
    ...
    prp(tree, extra=1, varlen = 0, main=main, box.col=cols, under=TRUE, 
        cex=0.4, round=0, split.font=1, ycompress = TRUE )
  }
...
```{r Fig11, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=5.5} 
