...
**15/4/15** wed 1.30-2.30 1hrs 3.45-
...
1. see setInputSensiScenarios
1. start to write script to run sensitivity analysis, probably by creating a large input object rather than a large input.params.csv.

1. write a function to calculate timeToThreshold for all scenarios based on listOut, allow user to specify which locus.

1. ask Ian if he has the code for the Barbosa classification trees analysis.

1. from the first stage of the sensitivity analysis I'll want to output a dataframe with 1 row per replicate containing both the inputs and the outputs. Getting it to put the inputs into a dataframe like this would be a good start.
