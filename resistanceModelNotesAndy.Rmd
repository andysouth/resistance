...
** 26/8/15 **
submit invoice
from contract : maximum of 25 days at 217 per day
...
~ run PRCC for difference between mix2 & sequential
~ might need to structure data differently



Tighten up the story before sending to Ian, I'm getting there. Maybe put into a summary Rmd pdf.



## IDEA
# rather than trees of e.g. mix1 better than sequence I could show time-to-resistance mix1-sequence.


OR I could even put time_to_resistance on the same plot for each stratgey (maybe with exposure on the x-axis)
This is very similar to some plots I have already, I just need to remove the facetting by strategy and remove individual strategies.

## BEWARE what am I doing with putting mixture better than seq on each repeated input ....  
## e.g. it doesn't seem to quite make sense that I have mixture or sequence better for insecticide1
## but maybe it is because all the inpiuts are repeated

## might be good to facet by mixture sequence better and just have points of x=exposure, y=effectiveness_insecticide1





~ do PRCCs on whether mixture or sequential better 

~ tomorrow when on good form in morning work out how best to summarise for Ian

~ add other resist thresholds into PRCC.
...
### My summary of results & thoughts

Resistance to two insecticides arises more slowly if they are used in combination rather than sequentially. See violin plots.




~ I'd like to show prcc on an axis from 0-1 with diff sysmbols for neg & pos. We care about which are most imp. params, not whether they are pos or neg.

~ If I would like to get prcc plots in a similar format to my existing plots. (but may not be space for axis labels)
~ get data in form : 
input1, input2, inputN, strategy 
prcc vals,,             indecticide1
...
# maybe add in a loop to plot for all 3 resistance thresholds
# maybe try faceting by parameter ranges ?
# lookup to see if I can steal any existing sensi analysis type code



See sensiAnPaperPartOneAtATime.r
...
Extended Curtis requests from Ian (still not quite sure how I will do)
...
1. look at investigating response by changing single parameters at a time (or can I use existing runs)
results files are in 'C:\\Users\\andy\\Documents\\resistanceResultsBackup\\listOutI1_1000.rda'
listOut2, & listOutMix

1. create a vignette that shows how best to run current model

1. I could ad something to shinyCurtis with a message at the top saying whether mixture or sequence better and maybe an arrow pointing to it.

1. I could add the combined mixture/individual (strategy 2) as a dotted line to plotcurtis_f2_generic()
(and then could even add labels of 1,2,3 to where they cross the 50% line)
...
### current state of play 18/12/15
...
Refactoring to arrays reduces code volume, reduces risk of mistyped variable names and allow objects to be passed between functions.
