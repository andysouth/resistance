...
Ian doesn't think IVCC ready to pull out checkbook again yet.
...
so to get mean fitness over time I would nead to multiply these by the genotype frequencies & would need to sort cis trans business + would need to have the frequencies of the different exposures (i think just a case of multiplying by the input exposure array (a))
...
done ~ get Matt's github fork up-to-date and create a folder for his work in it
~ look at linkage disequilibrium calc with Matt (maybe I could adapt one of my UIs to show it side by side with the freq plot)
...
~ talk to Bayer guy
~ how will we start the open malaria runs

linkage diseq:
~ where should I get Matt to save the new linkage diseq. outputs ?
~~ 
~ plotlinkage()
~ ld calc is in runModel2() ~line 390
~ i should add separate lines for M&F to linkage plot
...
~ invoice LSTM for June (see time below) + train + check whether I claimed Manchester hotel.
