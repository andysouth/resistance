...
### TO DO  
~ check stackoverflow for getting mean into hexbin plot

~ communicate my concern to Ian that figures I've spent quite a long time on just go into supplementary material where few will see them. Can we think of a way of getting these into a paper themselves ? e.g. if this first paper is targeted at policy people can we get a 2nd paper that is targetted at biologists and explores mechanisms ?


~ can I change order of strategies in legend ?
~ can I facet the smoothed scatter plots to get them to take up less space and avoid repeating legend ?
~~ or I may be able to use a similar plot_grid() method

~~ if I wanted to facet by inputs might need to restructure the data, put inputs into a single column to replace below :
  for(i in names_inputs)
  {
    print( ggplot(ggSubset, aes_string(x=i


~ Ian wants trees. Maybe just do one tree of resistance0.25 on whether seq or maxAdapt is better. 

adaptive & sequential
20% longer


~ start creating a proposed slimmed down figs doc (maybe as a word doc using pandoc) proposed for the paper
~~ beware that if I send it to Ian as a doc, he will insert text into it that I may not be able to find.
~~ maybe send it as a pdf first & change to a doc later to allow Ian to copy & paste the figs.
~~ see paper1_results_figs_slimmed.Rmd

~ I could try a 2D plot with x=exposure, y=effectiveness & hexbin & colour cells according to proportion of runs where mix or seq is better.

~~ seq is sometimes better than mix both. Where does that occur. The above plot might even help to answer that.
