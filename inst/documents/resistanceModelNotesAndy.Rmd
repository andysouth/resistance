...
# Weds 
~ look at adding a couple of simple unit-tests of some of the helper functions (& probably disable the expect_equal with runModel())

~ make edits to paper2
~ send msg to Ian & Matt




send msg to matt & ian about tidied code, include mention of the following :

~ say that I've been tidying & rearranging runModel2() to prepare for macro-mosaic & 3-locus options (which are now revealed both to be quite big jobs on the scale of weeks rather than days) 
...
~ can Matt check that linkage results are unchanged ?
