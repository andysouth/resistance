...
1. Questions for Ian
+ do we want to do sequential & mixtures for the first sensitivity analysis ?
+ what are the maximum generations that we are running for ? How do we deal with runs where critical point isn't reached in max generations.
+ is it OK to asses just resistance in males ?


1. write out how I'm going to run the sensitivity analysis so I can do a final check with Ian while I'm in Liverpool.

1. think about whether we are able to run sequential insecticide scenarios with the current setup.
