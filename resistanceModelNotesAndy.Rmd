...
1. continue with getting classification trees working in sensiAn1

1. update R, new lengths() function may be useful for me
but doesn't find latest version yet
library(installr)
updateR()
...
1. from the first stage of the sensitivity analysis I'll want to output a dataframe with 1 row per replicate containing both the inputs and the outputs. Getting it to put the inputs into a dataframe like this would be a good start. I think this may just be a case of transposing the input object ?

1. try out classification trees code from Susana
