...
~ tweak start of intro to make the justification stronger ...

~ do I need to add cost to the UI ?

~ get a version of the paper ready to send to IVCC and Justin before thursday
~ remove speculative & extra refs material put it in this doc



~ I need to be clear about the justification & target audience for paper2 early in the intro
~~ the abstract already does a pretty good job of that.

~ have a look at the malaria journal UI paper Katherine was involved with
~ keep it simple, don't get too carried away explaining intricacies of results
~ mention how this model is consistent with previous (more technical & less accessible) modelling results



~ have a quick look at new version barbosa paper

*may be best to put cost in now, this is quite a nice result, and is something new not covered in paper1*

*just include the extra figures, showing that cost increases time for a single insecticide and tends to increase time more for a mixture than a sequence. (Because slowing the resistance to one insecticide increases it's 'protection' of the other insecticide in a mixture.)*

~~ replace createArray2 with array_named() throughout resistance code

paper2

~ maybe I need to specify what I mean by 'mechanisms' in contrast e.g. to resistance mechanisms

check out :
Whalon, M.E., Mota-Sanchez, D & Hollingworth, R.M. (eds.) 2008. Global pesticide resistance in arthropods. CABI, Wallingford. )

Zhao JZ, Cao J, Li YX, Collins HL, Roush RT, Earle ED and Seltion AM, Transgenic
plants expressing two Bacillus thuringiensis toxins delay insect resistance evolution.
Nature Biotech 21:1493-1497 (2003). 

get my thinking straight on poor application, sub-lethal doses ...
related to RS survival, if RS survive resistance will be faster ...
will depend on dominance ...
and on resistance restoration see FAO p17.

My thinking (and seems to be Justins too) is that insecticide treatments will always be imperfect, so there are always going to be resistants that survive.

Poor application seems likely to reduce the selection pressure on resistance by reducing the number of susceptibles that are killed.
...
~ add early in paper that we are only modelling development of resistance and not the control of mosquitoes or malaria. Say that best option in purely resistance terms is not to use insecticide at all, but there would be be vector and malaria consequences.

~ in discussion mention migration, if susceptible individuals are migrating in this will reduce rate of resistance development.

~ Justin 1/12/16 said that they hear the msg that sub-lethal dosing accelerates resistance

~ what do I want the main msgs to be ?
...
~ rewrite text that refers to the first 4 panel fig
~ include cost of resistance ?
~ think about a diagram to explain the different inputs
~ don't get too sucked into explaining mechanisms precisely
~ discuss the 50% threshold and how it is quicker to get from 50 to 100 than to reach 50

~ key point in our Fig1B and from conventional wisdom is that poor insecticide application can promote rise in resistance. But I don't think we really cover this in how we run the model. E.g. increasing effectiveness and exposure (which one would expect to be related to insecticide application) all result in more rapid resistance. Are we saying that the insecticide application effect occurs purely because poor application results in higher dominance ? I should discuss this in paper2. Can link it to specific statements in GPIRM about poor application promoting resistance (if there are any). Could we add an insecticide application param ? 


~ check plot_fit_variance() & what it would do if multiple scenarios

~ ? should there be a dominance associated with the cost of resistance ? see 498 of revised MS
~ note that dominance is more relevant at low frequencies where much of the resistance is present in heterozygotes. Once much of the resistance is in homozygotes dominance is less relevant. (our LD output may be related to the proportion of R genotypes that are homozygous ...)
...
~~ think how we can address Justins question
~~ What does it mean if there is LLIN use and IRS deployment ?
...
Tangent : check whether I have power of attorney for M&D
...
~ think of a good way to present R-S calc

# now paper1 is nearly accepted I need to get a move on with paper2
# get to bottom of the different effects of effectiveness & exposure
...
WOuld probably just need to write a short script that queries ggSubset or similar to get input values and creates a new filed to store the new potential predictor.

~ 17/10/16 I had a slight concern that we might not normalise the fitness calcs where we should do, because of whats coming out in the effectiveness vs exposure investigation

~ The main figures that I use in paper2 too. Could they be modified to work in black & white ? Curtis had I1 dashed, I2 dotted and arrows to which are a mixture.
I might want to save this for paper2 ? Check what they look like in b&w.
Indeed is there any other way that they could be improved too ? perhaps by putting them closer together & avoiding repetition of axis labels.

In paper2 setting the x-axis constant would help with interpretation. Could set it to 100 for all except figs 11 A&B which I could set to 150.


~ can I allow lable of just m & s horizontally for mix & seq in plotcurtis_f2_generic ?
~ can I create the plot grid that I drew out on paper for paper2. Will the plots show up when made smaller ? Do I need to fiddle with plot boundaries ? Might be able to remove x labels from all but the lower row.



~ might I want to combine fitvis & expovis ?



~ can I setup something clear for Matt to do that will be useful for us ?
~ ask him to work out the arithmetic about effectiveness & exposure that Ian started when we were in liverpool.


   
~ I could use plot_fit_rs() to produce some figs for paper2 on how inputs influence fitness maybe initially for single locus ? then to include exposure need to show genotype popn fitness.   

~ do I completely have it straight in my head what the model is doing re niches ? Yes I think I do now.

~ confirm best terminology for fitness calcs.

~ in the code I use NicheFitness, then multiplied by exposure to give GenotypeFitness, I don't think either naming scheme is quite right yet ... locusFitness is calcualted for each niche too ...

Locus fitness : 	SS = 1-effectiveness
		RS = 1-effectiveness + (dominance*selection coeff.)
		RR = 1-effectiveness + selection coeff.

Allele fitness : 	SS1SS2 = SS * SS
		SS1RS2 = SS * RS
		…
		RR1RR2 = RR * RR

Genotype fitness = Allele fitness * exposure to insecticide
...
to :
loci=c('SS1','RS1','RR1','SS2','RS2','RR2','SS3','RS3','RR3'), exposure=c('no','lo','hi')
...
Set up more ability to be able to see what is going on with fitnesses from the code.
Maybe I could even add this to one of my UIs. The inputs will be the same.
Could put the new outputs in together or in a separate UI.
...
~ look at colin gillespies efficient R prog book on github
...
~ future : would be cool to have a graphical description of the core model, e.g. effectiveness, exposure, selective advantage, fitness ...

# best way to allow setting of multiple scenarios varying one param at a time ?

# can I make any smaller components from get_resistance()
...
# Matt needs to do this each time to sync his fork to get updates from me
git fetch upstream
git checkout master
git merge upstream/master
...
_CUT bits from paper2 to go somewhere else :_
