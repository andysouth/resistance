...
1. why can't shinyapps find resistance for this :
Deployment completed: https://andysouth.shinyapps.io/shinyFig2Curtis
Warning message:
In FUN(X[[i]], ...) :
  Package 'resistance' not available in repository or locally

1. work out what to do if critical threshold not reached in findResistancePoints()
causes : Error in plot.window(...) : need finite 'xlim' values

1. CHECK MODEL RUNS ARE DOING WHAT I THINK. Currently seems that resistance is quicker for mixtures under all scenarios (or very nearly all)

1. create findResistancePointsMixResponsive()

1. review how to answer these questions.
...
1. work further on sensiAnPaper1All 

1. decide whether to do main runs with P_2 dependent on P_1 or not
