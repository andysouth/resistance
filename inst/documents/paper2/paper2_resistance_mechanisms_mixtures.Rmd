...
The model outputs the change in resistance allele frequencies over time (generations). We use this to calculate a time-to-resistance using a resistance threshold of 50%. This is consistent with thersholds used in previous modelling analyses [todo refs] and represents a level where the resistance alleles are close to fixation.
...
When single insecticide use was investigated (Figs 3-4) higher values of inputs that increased the selective advantage of resistance all led to more rapid evolution resistance, and higher values for cost led to slower evolution of (Fig 4B). The latter is as expected because it reduces the selective advantage of resistance. 
...
This model has been designed to be flexible to address further issues in Insecticide Resistance Management. With a little restructuring our model can represent other insecticide use strategies such as rotations and mosaics.


#### FINAL CONCLUDING PARAGRAPH

(@) Effectiveness and exposure are key and can be modified. Cost also has an effect and future work may want to look more at it's interaction with these other important inputs.
