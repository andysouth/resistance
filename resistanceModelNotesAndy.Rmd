...
In max(combmat[, gencol]) : no non-missing arguments to max; returning -Inf
fixed by returning listOut from runModel()

**17/3/15** tues to liverpool 8.30-5.30 9hrs

done: add a savePlots argument, to enable plotting without saving
...
**17/3/15** weds in liverpool 8.45-
...
### TO DO  
1. add myself as maintainer in DESCRIPTION
1. sort location in package of "input.parameters.csv" functions should be able to run without it
