...
Check once more then,
Reply to Ian, say it looks do-able, and should just take a few days.
1. what range do we want for effectiveness ? stick to 0.45-1, or we could go lower e.g. 0.3-1.
2. more thought son what we can call the new input to fit on figs. e.g. resist_coeff_I1 ?  

Is there potential to rationalise the analysis before re-running ?

RR_restoration_coefficient or resistance_coeff_I1 & resistance_coeff_I2
What I will need to do : search 14/6/16 to see changes
#sensiAnPaperPart.r
done ~ add generation of new inputs & multiplication to give s
started ~ save new inputs to input file to enable post-run analysis
...
#? which script to use as a starting point for post run analysis ?
#paper1_results_figs_slimmed50.Rmd
or
#paper1_results_figs.Rmd
~ change loading of model outputs
~ replace 's*' with resistance_coeff_I1 in this line
treePredictors <- c('P_1','P_2','exposure','phi.SS1_A0','phi.SS2_0B','h.RS1_A0','h.RS2_0B','s.RR1_A0','s.RR2_0B')
treePredictors <- c('P_1','P_2','exposure','phi.SS1_A0','phi.SS2_0B','h.RS1_A0','h.RS2_0B','resistance_coeff_I1','resistance_coeff_I2')
...
~ modify curtisInputs to include r rather than s  
  

Beware that although I may pass resistance_coeff_I1 to runModel2() it will not be used, it will just go in the inputs to enable post run analysis. s.RR1_A0 will be used by the model. I could perhaps put in a check that s=newR * effectiveness.


Qs for Ian before new analysis
1. what range do we want for effectiveness ? stick to 0.45-1, or we could go lower e.g. 0.3-1
2. eventually would we need diff values for high & low exposure
