...
1. Exposure has the greatest effect on time-to-resistance, but as it has a similar effect across all strategies it has little influence on which strategy is preferable.
...
# Difference between time-to-resistance for sequential and mixture2

```{r, eval=TRUE, echo=FALSE, warning=FALSE} 

#names_results <- c("time_to_resistance0.5","time_to_resistance0.25","time_to_resistance0.1")
# names_results <- c("mix2_minus_seq0.25")
#   
# ggSubset <- df_diff #[ !ggInsOuts$strategy %in% c("insecticide 2"), ]  
#   
# for(i in names_results)
# {
#   #x11()
#   
#   print( ggplot(ggSubset, aes_string(y=i)) + 
#       #ylim(0,450) +
#       #coord_cartesian( ylim=c(0, 350)) +
#       geom_violin(draw_quantiles = c(0.25, 0.75), show.legend=FALSE, adjust = .5 )
# 
...
## How difference in time-to-resistance for sequential and mixture3 is effected by input values
...
```{r, eval=TRUE, echo=FALSE, fig.height=3.5} 
  ...
  for(i in names_inputs)
  {
    ...
    print( ggplot(ggSubset, aes_string(x=i, y=y, colour="gen_cP0.25seq")) + #, colour="strategy")) + 
            #points not wanted if 10000
             geom_point(shape=3, show.legend=FALSE) + 
             #geom_smooth(colour='red', linetype='dashed',size=0.5) +
             geom_smooth(linetype='dashed',size=1) +
