...
```{r, eval=FALSE}
  ...
  save(listOutMix, file=paste0(outFolder,'listOutMix_ex1000.rda'))
  save(listOutI1, file=paste0(outFolder,'listOutI1_ex1000.rda'))
  save(listOutI2, file=paste0(outFolder,'listOutI2_ex1000.rda'))  
...
### investigating response of time-to-resistance to individual input parameters
### (while they all are varying so it may not work well)
```{r, eval=TRUE} 
  ...
  #this one shows no discernible pattern
  ggplot(ggInsOuts, aes(x=effectiveness_insecticide1, y=gen_cP0.5, color=strategy)) + geom_point() + facet_wrap( ~ strategy) + geom_smooth()
  ...
  #or even subset the data for specific parameter ranges
