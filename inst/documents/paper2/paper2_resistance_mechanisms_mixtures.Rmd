...
1. ? briefly refer to linkage disequilibrium and say we are investigating OR include a figure OR refer to one of my existing figures
1. can I develop explanatory diagrams to describe the input parameters ?
1. is there a reference to say that the effectiveness of insecticides can be modified at the design stage ?
...
Public health efforts in controlling malaria have centred on treated bed nets and indoor residual spraying, both reliant on insecticides. Treated nets were recently estimated to contribute 68% and indoor residual spraying 13% to averting more than 500 million malaria cases between 2000 and 2015 [@Bhatt2016]. 

A recent malaria transmission model [@Churcher2016] predicts that even low pyrethroid resistance is likely to increase malaria incidence in Africa by reducing the performance of bed nets.

The Innovative Vector Control Consortium (IVCC) was set up in 2005 to develop new vector control tools and particularly new insecticides to address insecticide resistance in malaria transmitting mosquitoes [@Hemingway2006][@IVCC2016]. Several new insecticides are now in development [@IVCC2016] and very soon decisions will need to be made about how best to use them. The model presented here can help inform strategies to minimise the developm,ent of resiostance in new and existing insecticides.
...
> Resistance allele frequency = 2*RR + RS
...
It is worth re-iterating that we are looking solely at the evolution of resistance and not at the control of mosquitoes or malaria. As such our model would predict that the best strategy for limiting the development of resistance in the short term would be to use no insecticide. Using no insecticide could have serious consequences for mosquito and malaria abundance. The challenge is to develop good strategies for delivering mosquito and malaria control in the short term while and sustaining that in the longer term by restricting the development of resistance. Others are looking at combining models of population genetics and population dynamics to investigate these trade-offs [@Barbosa2017].
...
#### discussion: putting results into context (some parts may want to go earlier)
I will now put these predicted responses of resistance into the context of previous work and recomendations.

Most of these predicted reponses of resistance are consistent with recomendations that have been produced by WHO[@WHO2012], IRAC[@IRAC2011] and FAO[@FAO2012].

[Next bit might want to go in intro, but not if I refer to our results]
Earlier modelling work mostly from the 1980s has made sometimes contrasting predictions and these can be explained by the details of their structure and the particular situations they represent. One model predicted that "the use of mixtures is always more effective in delaying the onset of resistance often by many orders of magnitude"[@Mani1985]. That model assumed that SS genotypes were always killed, RR always survived, and that a constant 10% of mosquitoes escaped the insecticide. This would be represented in our model by a very high 'insecticide effectiveness' of 1, a 'resistance restoration' of 1 and an 'exposure' of 0.9. As described earlier we particular predict that the very high effectiveness is likely to favour mixtures over sequences.

[@Roush1989]

The model we describe is more flexible and as such can reproduce similar results to these earlier approaches and be used to represent more detailed, operationally relevant strategies. 
...
_todo mention other potential effects e.g. costs of resistance, sex linkage, ..._

_TODO : Should I try to cite in press Killeen about combining interventions ?_

TODO FINAL CONCLUDING PARAGRAPH
~ Effectiveness and exposure are key and can be modified.
~ The modelling approach can be used to investigate more detailed strategies.
...
![Screenshot of one online model user interface, accessible at : https://andysouth.shinyapps.io/resistmob2/](paper2_UI1.png)

```{r Fig2, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.height=7.5, fig.width=7.5, fig.show='hold', fig.align='left', fig.cap="Effect of inputs on changing resistance frequency over time with single insecticide use. A. Effectiveness, B. Exposure, C. Dominance, D. Resistance restoration. For each panel the chosen input was varied in isolation with the remaining inputs set to 0.5, except for starting frequency of resistance which was set to 0.01). "}
...
```{r Fig3, eval=TRUE, echo=FALSE, fig.height=3.7, fig.width=3, fig.show='hold', fig.align='left', fig.cap="Effect of changing resistance starting frequency on changing resistance frequency over time with single insecticide use."}
...
```{r Fig6, eval=TRUE, echo=FALSE, fig.height=7, fig.width=7, fig.show='hold', fig.align='left', fig.cap="A. All control inputs equal at 0.5, B. Resistance restoration for insecticide1 increased from 0.5 to 0.8, C. Dominance for insecticide1 increased to 0.8, D. Resistance restoration and dominance for insecticide1 increased to 0.8"}
...
```{r Fig7, eval=TRUE, echo=FALSE, fig.height=7, fig.width=7, fig.show='hold', fig.align='left', fig.cap="A. All control inputs equal at 0.5, starting frequencies of resistance at 0.01, B. Effectiveness for insecticide1 increased from 0.5 to 0.8, C. Starting frequency of resistance for insecticide1 decreased from 0.01 to 0.001, D. Effectiveness for insecticide1 increased from 0.5 to 0.8 and starting frequency for insecticide1 decreased from 0.001 to 0.0001"}
...
```{r Fig8, eval=TRUE, echo=FALSE, fig.height=7, fig.width=7, fig.show='hold', fig.align='left', fig.cap="A. All control inputs equal at 0.5, male exposure 0.8, B. Effectiveness for insecticide1 increased from 0.5 to 0.8, C. Male exposure decreased from 0.8 to 0.2, D. Effectiveness for insecticide1 increased from 0.5 to 0.8 and Male exposure decreased from 0.8 to 0.2"}
